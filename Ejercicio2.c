...
int main() {
    ...
    // Simulación del pipeline con manejo de dependencias
    for (int ciclo = 0; ciclo < numInstrucciones + NUM_ETAPAS - 1; ciclo++) {
        for (int i = 0; i < NUM_ETAPAS; i++) {
            if (ciclo - i >= 0 && ciclo - i < numInstrucciones) {
                // Manejo de dependencia: si una instrucción LOAD es seguida por ADD o SUB, insertamos un STALL
                if (i > 0 && strcmp(instrucciones[ciclo - i], "LOAD") == 0) {
                    if (ciclo - i + 1 < numInstrucciones &&
                        (strcmp(instrucciones[ciclo - i + 1], "ADD") == 0 ||
                         strcmp(instrucciones[ciclo - i + 1], "SUB") == 0)) {
                        strcpy(pipeline[i], "STALL");  // Ciclo de espera
                    } else {
